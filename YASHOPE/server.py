...
class User():
	...
	def register(login, password):
		...
		return {
			"ok": True,
			"message": "Success: Registerted!"
		}


class GetPage():

	def success(message="It was made!!<br>Congratulations!", isAuth=None):
		if not isAuth:
			isAuth = User.isAuth()
	...
	def error(e="Something went wrong"):
		resp = make_response(render_template(
	...
	def ofProduct(id):
		...
		if id in data:
			...
			resp = GetPage.error("No such page")
		return resp
	...
	def edit_product(id):
		...
		data = json.load(io.open('data/data.json', 'r', encoding='utf-8-sig'))
		if id not in data:
			return GetPage.error("No such page")
		name = data[id]["name"]
		descr = data[id]['descr']
	...
	def add_item(ident=""):
		...
		if added["ok"]:
			...
			return GetPage.success("The page was updated!")
		else:
	...
	def register():
		...
		if reg["ok"]:
			...
			if session["ok"]:
				resp = GetPage.success("You are our member now!!", isAuth=1)
	...
	def auth():
		...
		session = User.start_session(
			login, password, str(request.remote_addr))
		if session["ok"]:
			resp = GetPage.success("You are authenticated!!", isAuth=1)
			resp.set_cookie("Auth", session["message"])
			return resp
		return GetPage.error(session["message"])
...
def cart():
	if User.isAuth():
		...
		return GetPage.error("Authorize first!!")
...
def edit_item(id):
	...
	if not isOwner:
		return GetPage.error("No access! You damn hacher")
